...
const AgentCashOutRequests = () => {
  const [cashOutRequests, setCashOutRequests] = useState(useLoaderData());

  const handleApprove = async (transaction) => {
    const response = await fetch(
      `${import.meta.env.VITE_SERVER_URL}/agent/approve-cash-out/${
        transaction._id
      }`,
      {
        method: "PUT",
    ...
    if (data.success) {
      Swal.fire({
        title: "Success!",
        text: `Cash Out Request of ${transaction.amount} tk from ${transaction.sender} Approved!`,
        icon: "success",
      });
    ...
    <section>
      ...
      <div className="relative overflow-x-auto shadow-md sm:rounded-lg">
        <table className="w-full text-sm text-center rtl:text-right text-gray-500 dark:text-gray-400">
          ...
          <tbody>
            {cashOutRequests.requests.map((item) => (
              <tr
                key={item._id}
